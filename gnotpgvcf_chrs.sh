...
#SBATCH --array=0-999
#SBATCH -o log/1kb_gnotpgvcf_%A_%a.out
#SBATCH -e log/1kb_gnotpgvcf_%A_%a.err

module load singularity

cd /gscratch/wasser/khan3/elephant_snps/data/gatk/gvcf

CHRS=($(cat ../remaining_chr.txt|head -1000))
...
if test -d "${CHR}"; then
	echo "${CHR} output directory exists"
elif test ! -d "${CHR}"; then
	mkdir ${CHR}
fi

FILE=($(ls -1 /gscratch/wasser/khan3/elephant_snps/data/gatk/interval_lists/seqstats/*_intervals.txt |grep "${CHR}_1kb"))
INTS=$(cat $FILE)
for INT in $INTS; do
